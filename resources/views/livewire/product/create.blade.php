        ...
        <div class="mt-5 p-4 relative z-10 bg-white border border-gray-200 rounded-xl sm:mt-10 md:p-10">
            <form wire:submit.prevent='addProduct' enctype="multipart/form-data">
                ...
                <div class="flex justify-center mt-5">
                    @if ($image)
                        <img src="{{ $image->temporaryUrl() }}" class="w-40 h-40 object-cover rounded" alt="">
                    @else
                        <img src="{{ asset('storage/placeholder-image.png') }}" class="w-40 h-40 object-cover rounded"
                            alt="">
                    @endif
                </div>
                ...
                <div class="mt-6">
                    ...
                    <div class="mt-1">
                        <input type="file" wire:model="image"
                            class="py-2.5 sm:py-3 block w-full border-gray-200 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none">
                    </div>
                    @error('image')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
